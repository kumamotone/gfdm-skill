class ReMusicsController < ApplicationController
  ...
  def show
    @music = ReMusic.find(params[:id])
  end

  def hot
    @musics = ReMusic.all.where(ishot: true)
  end

  def other
    @musics = ReMusic.all.where(ishot: false)
  end
  ...
  def destroy
    entry = ReNusic.find(params[:id])
    client = ApplicationController.helpers.get_twitter_client

    tweet_text = "曲情報が削除されました。(by @#{current_re_user.twitterid}) \n\n削除曲: #{entry.name}"
  ...
  def update
    ...
    if @music.update_attributes(music_params)
      ...
      tweet_text = "曲情報が変更されました。(by @%s) \n\n%.39s\nD %.2f %.2f %.2f %.2f\nG %.2f %.2f %.2f %.2f\nB %.2f %.2f %.2f %.2f" % [current_re_user.twitterid , @music.name, @music.d_bsc, @music.d_adv, @music.d_ext, @music.d_mas, @music.g_bsc, @music.g_adv, @music.g_ext, @music.g_mas, @music.b_bsc, @music.b_adv, @music.b_ext, @music.b_mas]
      if Rails.env == 'development'
        tweet_text = tweet_text + "(開発テスト)"
      end
      client.update(tweet_text)
  ...
  def create
    @music = ReMusic.new(music_params)
    @music.id = ReMusic.maximum(:id).next  # 最大の番号が削除済みの場合連番にならなくなってしまうので調整
    @music.name.strip! unless @music.name.nil?
  ...
  def music_params
    params.require(:re_music).permit(:name, :bpm, :d_bsc, :d_adv, :d_ext, :d_mas,
  ...
  def admin_user
    if current_re_user.admin? || current_re_user.subadmin?
      # pass
    else
