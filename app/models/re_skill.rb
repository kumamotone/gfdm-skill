class ReSkill < ActiveRecord::Base
  belongs_to :re_user
  belongs_to :re_music
  validates :re_user_id, presence: true
  validates :comment, length: {maximum: 100}
  validates :re_music_id, presence: true, uniqueness: {scope: [:user_id, :kind]}
  ...
  def self.import(file, id)
    ...
    open(file.path, 'r:cp932:utf-8', undef: :replace) do |f|
      ...
      csv.each do |row|
        ...
        skill = find_by(:re_music_id => table["music_id"], :kind => table["kind"], :re_user_id => id)
        if skill.nil?
          skill = new
          skill.user_id = id
        end
    ...
    ApplicationController.helpers.re_updatedrum(id)
    ApplicationController.helpers.re_updateguitar(id)
